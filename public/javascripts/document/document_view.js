//TODO: REMOVE THIS PEACE OF PAIN
DocViewPlace = function($page, doc) {
  var $cont = $page.find('[name=content]');
  ...
  var drowSelectedLexem = function(it){
    if (typeof it != 'string' || it.charAt(0) != '!') return;
    var v = LexemeRegExp.exec(it.slice(1));
    return Templates['doc/lexS']({name: v[1], descr: v[2]});
  };
  ...
  var drowAudio = function(it){
    if (! doc.audio) return;
    if (it._type == 'audio' && doc.audio[it.id]) {
      return Templates['doc/audio'](doc.audio[it.id]);
    }   
  };
  var drowLexemSet = function(it){
    ...
    var tn = 'doc/lex';
    if (it.selected) tn += 'S';
    return Templates[tn]({name: it.name, descr: descr.join('|')});
  };
